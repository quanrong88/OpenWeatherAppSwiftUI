...
public class OpenWeatherService {
    ...
    public func getLocation(q: String) -> AnyPublisher<LocationResponse, MoyaError>  {
        provider.requestPublisher(.getLocation(q: q))
            .map(LocationResponse.self)
    }
    
    public func getCurrentWeather(lat: Double, lon: Double, unit: Unit = .metric) -> AnyPublisher<CurrentWeatherResponse, MoyaError> {
        provider.requestPublisher(.getCurrentWeather(lat: lat, lon: lon, unit: unit)).map(CurrentWeatherResponse.self)
    }
    
    public func getWeatherForecast(lat: Double, lon: Double, unit: Unit = .metric) -> AnyPublisher<WeatherForecastResponse, MoyaError> {
